...
describe('test palindrome (string reversal)', function() {
  it('should reverse string using built in functions', () => {
    ...
    expect(fn.reverseStringOneLiner('njnschnjkdasn j32 uida')).toEqual(
      'adiu 23j 21utkubmit'
    )
  ...
  it('should reverse string iteratively', () => {
    ...
    expect(fn.reverseStringIterative('njnschnjkdasn j32 uida')).toEqual(
      'adiu 23j 21utkubmit'
    )
    expect(fn.reverseStringIterative('timbuktu12')).toEqual('21utkubmit')
    expect(fn.reverseStringIterative('')).toEqual('')
    expect(fn.reverseStringIterative('reverseastring')).toEqual(
      'gnirtsaesrever'
    )
  })
  ...
  it('should reverse string recursively', () => {
    ...
    expect(fn.reverseStringRecursive('njnschnjkdasn j32 uida')).toEqual(
      'adiu 23j 21utkubmit'
    )
    expect(fn.reverseStringRecursive('timbuktu12')).toEqual('21utkubmit')
    expect(fn.reverseStringRecursive('')).toEqual('')
    expect(fn.reverseStringRecursive('reverseastring')).toEqual(
      'gnirtsaesrever'
    )
  })
})
