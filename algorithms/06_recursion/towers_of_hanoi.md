...
var diskText= function(disknum,largestdisk){
    ...
    var diskstring="";
    var i;
    for(i=0;i<leftspaces;i++){ diskstring += " "; }
    if(disknum === 0){ diskstring += " "; }
    else{ 
        for(i=0;i<(diskwidth-1)/2;i++){ diskstring += "*"; }
        diskstring += disknum;
        for(i=0;i<(diskwidth-1)/2;i++){ diskstring += "*"; }
    }
    for(i=0;i<rightspaces;i++){ diskstring += " "; }
    return diskstring;
};

//this displays the current state of the puzzle
var displayHanoi=function(){
    if(badmove){ return; } //stop displaying after an illegal move
    var totalDisks= pegs[0].length + pegs[1].length + pegs[2].length;
    for(var i=totalDisks-1 ;i >=0;i--){
        var rowText="";
        for(var k=0;k<=2;k++){
            var disk=0;
            if(i < pegs[k].length){ disk=pegs[k][i]; }
    ...
    var basestring="";
    var j=0;
    for(j=0;j< totalDisks+1;j++){ basestring += "-";}
    basestring += "A";
    for(j=0;j< 2*totalDisks+2;j++){ basestring += "-";}
    basestring += "B";
    for(j=0;j< 2*totalDisks+2;j++){ basestring += "-";}
    basestring += "C";
    for(j=0;j< totalDisks+1;j++){ basestring += "-";}
...
var moveDisk=function(fromPeg,toPeg){
    if(badmove){return;} //reject any more moves after an illegal move
    ...
    if(fromPegEmpty === "From peg is not empty"){
        ...
        var fromDiskSmaller=(toPegEmpty||(disk < topDiskOnToPeg)) ? "From disk smaller" : "From disk larger";
        ...
        if(fromDiskSmaller === "From disk smaller"){
            ...
            badmove=true;
            println("You tried to put a disk from peg "+fromPeg+ " of size "+disk +
                    " on top of a disk of size "+ topDiskOnToPeg + " on peg " + toPeg+"\n\n");
        }
    }else{
        badmove=true;
...
var getSparePeg=function(firstPeg,secondPeg){
    if(firstPeg !== "A" && secondPeg !== "A"){ return "A";}
    if(firstPeg !== "B" && secondPeg !== "B"){ return "B";}
    return "C";
};
